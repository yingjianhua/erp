...
public class PurRtnGrf extends GrfForm implements SheetVarsInf {
	static {
		PurRtn.TB.getCode(); PurRtnLine.TB.getCode(); SysOrg.TB.getCode(); SysCom.TB.getCode(); SysShiping.TB.getCode(); SysPersonLink.TB.getCode(); GsWarehouse.TB.getCode(); SysUser.TB.getCode(); SysEm.TB.getCode(); SysPerson.TB.getCode();
	}
	private static final PurRtn.T T = PurRtn.T.PKEY;
	private static final PurRtnLine.T L = PurRtnLine.T.PKEY;
	...
	public static void main(String[] args) {
		String mainParams[] = toCodes(new VFlds(T.ORG, T.CODE, T.SUPPLIER, T.SUPNAME, T.STATUS, T.AMT, T.AMT_COST,
				T.WAREHOUSE, T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.REM),
		...
		new CrtJsp(mainParams, linesParams, PurRtn.class, PurRtnLine.class).run();
	}

	public PurRtnGrf() {
		super(PurRtn.class, "采 购 退 货 单", 8, 
				new VFlds(T.ORG, T.CODE, T.SUPPLIER, T.SUPNAME, T.STATUS, T.AMT, T.AMT_COST, T.WAREHOUSE,
						T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.REM, COM.TEL1, COM.ADDR, COM.FAX, LINK.NAME, LINK.PE_MOBILE));
	...
	// ph:PageHeader, pf:PageFooter, rh:ReportHeader, rf:ReportFooter
		...
		public void initItemsSection(GrfItemsSection is, GrfSectionItem ph,
			...
			rh.AddItem("StaticBox", "orderName", "采 购 退 货 单", line2.cellLocationSizeAbs(1, 2, 2, 1)).setTextAlignToMiddleCenter().add(FONT_REPORT_HEAD);
			...
			rh.AddName(LINK.NAME, line6.cellLocationSizeAbs(1, 3)).addAttr("BorderStyles", new GrfExp("[DrawTop]"));
			rh.AddValue(LINK.NAME, line6.cellLocationSizeAbs(1, 4), new VFlds(T.SUPPLIER), tbs, personLinkName).addAttr("BorderStyles", new GrfExp("[DrawTop]"));
			rh.AddName(LINK.PE_MOBILE, line6.cellLocationSizeAbs(1, 5)).addAttr("BorderStyles", new GrfExp("[DrawTop]"));
			rh.AddValue(LINK.PE_MOBILE, line6.cellLocationSizeAbs(1, 6), new VFlds(T.SUPPLIER), tbs, personLinkPeMobile).addAttr("BorderStyles", new GrfExp("[DrawTop|DrawRight]"));
			Sheet line7 = rh.getSheet().newInnerSheet(7, 1, new double[] {0.65}, 2.65, 7.2, 2.5, -1);
			rh.AddItem("StaticBox", "rh1-1", "联系人/手机:", line7.cellLocationSizeAbs(1, 1)).addAttr("BorderStyles", new GrfExp("[DrawLeft|DrawBottom]"));//空这占位
			rh.AddItem("StaticBox", "rh1-2", " ", line7.cellLocationSizeAbs(1, 2)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));//空这占位
			rh.AddItem("StaticBox", "rh1-3", "采购员/电话:", line7.cellLocationSizeAbs(1, 3)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));//空这占位
			rh.AddItem("StaticBox", "rh1-4", " ", line7.cellLocationSizeAbs(1, 4)).addAttr("BorderStyles", new GrfExp("[DrawRight|DrawBottom]"));//空这占位
...
//			rh.AddValueMulti(T.BUYER, line7.cellLocationSizeAbs(1, 4), new VFlds(T.BUYER), new VFlds(T.BUYER, BUYER.TB_OBJ, EM.PERSON, PERSON.OF_TEL)).addAttr("BorderStyles", new GrfExp("[DrawRight|DrawBottom]"));
			rh.AddItem("StaticBox", "orderlist", "退货清单:", 8, 1).add(FONT_14_BOLD).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
...
//			rf.AddValue(T.REM, fline1.cellLocationSizeAbs(1, 2)).addAttr("BorderStyles", new GrfExp("[DrawTop|DrawRight|DrawBottom]"));
			...
			rf.AddItem("StaticBox", "fhsj", "发货时间:", fline2t9.cellLocationSizeAbs(2, 2));
			rf.AddItem("StaticBox", "fhsj2", " ", fline2t9.cellLocationSizeAbs(2, 3)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
...
//			rf.AddValue(SHIP.TIME_SHIP_PLAN, fline2t9.cellLocationSizeAbs(2, 3), new VFlds(T.SHIPING, SHIP.TIME_SHIP_PLAN)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
			...
			rf.AddItem("StaticBox", "ysfs", "运输方式:", fline2t9.cellLocationSizeAbs(3, 2));
			rf.AddItem("StaticBox", "ysfs2", " ", fline2t9.cellLocationSizeAbs(3, 3)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
...
//			rf.AddValue(T.SHIPING_MODE, fline2t9.cellLocationSizeAbs(3, 3)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
			...
			rf.AddItem("StaticBox", "jhdz", "交货地址:", fline2t9.cellLocationSizeAbs(4, 2));
			rf.AddItem("StaticBox", "jhdz2", " ", fline2t9.cellLocationSizeAbs(4, 3)).addAttr("BorderStyles", new GrfExp("[DrawBottom]"));
	...
	private static class MyDetailGrid extends GrfFormLine {
		public MyDetailGrid(GrfForm parent) {
			super(
					...
					new double[] { 1, 3, 3.9, 5.6, 1, 4.3, 4, 3.9 },
					null,
					new VFlds(L.GOODS, GOODS.CODE),
					new VFlds(L.GOODS, GOODS.NAME),
					new VFlds(L.GOODS, GOODS.SPEC),
