...
public class PurOrderDirectGrf extends GrfForm implements SheetVarsInf {
	static {
		PurOrderDirect.TB.getCode(); PurOrderDirectLine.TB.getCode(); SysOrg.TB.getCode(); SysCom.TB.getCode(); SysShiping.TB.getCode(); SysPersonLink.TB.getCode(); SysUser.TB.getCode(); SysEm.TB.getCode(); SysPerson.TB.getCode();
	}
	private static final PurOrderDirect.T T = PurOrderDirect.T.PKEY;
	private static final PurOrderDirectLine.T L = PurOrderDirectLine.T.PKEY;
...
//	private static final VFlds[] personLinkPeMobile = {new VFlds(LINK.PE_MOBILE)};
	...
	public static void main(String[] args) {
		String mainParams[] = toCodes(new VFlds(T.ORG, T.CODE, T.SUPPLIER, T.SUPNAME, T.STATUS, T.AMT, T.AMT_COST, T.REV_ADDR,
				T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.SHIPING_MODE, T.BUYER, T.REM),
		...
		mainParams = toCodes(mainParams, toLinkCode(new VFlds(T.SUPPLIER), tbs, personLinkFax));
		...
		new CrtJsp(mainParams, linesParams, PurOrderDirect.class, PurOrderDirectLine.class).run();
	}

	public PurOrderDirectGrf() {
		super(PurOrderDirect.class, "采 购 直 销 单", 8, 
				new VFlds(T.ORG, T.CODE, T.SUPPLIER, T.SUPNAME, T.STATUS, T.AMT, T.AMT_COST, T.REV_ADDR,
...
//		getVFld(LINK.PE_MOBILE).setName("手机");
	...
	// ph:PageHeader, pf:PageFooter, rh:ReportHeader, rf:ReportFooter
		...
		public void initItemsSection(GrfItemsSection is, GrfSectionItem ph,
			...
			rh.AddItem("StaticBox", "orderName", "采 购 直 销 单", line2.cellLocationSizeAbs(1, 2, 2, 1)).setTextAlignToMiddleCenter().add(FONT_REPORT_HEAD);
