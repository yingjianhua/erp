...
public class GsOutGrf extends GrfForm implements SheetVarsInf {
	...
	// 设置静态全局常量
	private static final GsOut.T T = GsOut.T.PKEY;
	private static final GsOutLineView.T L = GsOutLineView.T.PKEY;
	...
	public static void main(String[] args) {
		...
		new CrtJsp(mainParams,linesParmas, GsOut.class, GsOutLineView.class).run();
	}

	// 设置报表头的参数
	public GsOutGrf() {
		super(GsOut.class, null, 8, new VFlds(T.CODE,T.WAREHOUSE,T.ORIG_FORM,T.ORIG_FORM_NUM,T.CREATED_TIME,T.REM,T.CREATED_BY,T.APPR_BY));
	...
	public void initItemsSection(GrfItemsSection is, GrfSectionItem ph, GrfSectionItem pf, GrfSectionItem rh, GrfSectionItem rf) {
		...
		// 设置静态框并给予值
		rh.AddItem("MemoBox", "orderName","[#{origForm.tb.name}#]出 库 单", line2.cellLocationSizeAbs(1, 2, 2, 1)).setTextAlignToMiddleCenter().add(FONT_REPORT_HEAD);
