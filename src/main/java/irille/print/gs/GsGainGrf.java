...
public class GsGainGrf extends GrfForm implements SheetVarsInf {
	static {
		GsGain.TB.getCode();
		GsGainLine.TB.getCode();
		SysOrg.TB.getCode();
		SysCom.TB.getCode();
		SysUser.TB.getCode();
		SysEm.TB.getCode();
	...
	// 设置静态全局常量
	private static final GsGain.T T = GsGain.T.PKEY;
	private static final GsGainLine.T L = GsGainLine.T.PKEY;
	...
	public static void main(String[] args) {
		...
		// 生成jsp文件run()
		new CrtJsp(mainParams, linesParmas, GsGain.class, GsGainLine.class).run();
	}
	...
	public void initItemsSection(GrfItemsSection is, GrfSectionItem ph, GrfSectionItem pf, GrfSectionItem rh, GrfSectionItem rf) {
		...
		// 设置静态框并给予值
		rh.AddItem("StaticBox", "orderName", "盘 盈 单", line2.cellLocationSizeAbs(1, 2, 2, 1)).setTextAlignToMiddleCenter().add(FONT_REPORT_HEAD);
	...
	// 设置报表头的参数
	public GsGainGrf() {
		super(GsGain.class, null, 8, new VFlds(T.ORG, T.CODE, T.STATUS, T.WAREHOUSE, T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.REM, COM.TEL1, COM.ADDR, COM.FAX));
