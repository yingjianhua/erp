...
public class SalPresentGrf extends GrfForm implements SheetVarsInf {
	static {
		SalPresent.TB.getCode();
	...
	private static final SalPresent.T T = SalPresent.T.PKEY;
	private static final SalPresentLine.T L = SalPresentLine.T.PKEY;
	...
	public static void main(String[] args) {
		String mainParams[] = toCodes(new VFlds(T.ORG, T.CODE, T.CUST, T.CUST_NAME, T.STATUS, T.AMT, T.AMT_COST, T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.SHIPING_MODE, T.OPERATOR, T.REM), new VFlds(T.ORG, ORG.COM, COM.NAME), new VFlds(T.ORG, ORG.COM, COM.ADDR), new VFlds(T.ORG, ORG.COM, COM.TEL1), new VFlds(T.ORG, ORG.COM, COM.FAX), new VFlds(T.SHIPING, SHIP.ADDR), new VFlds(T.SHIPING, SHIP.NAME), new VFlds(T.SHIPING, SHIP.MOBILE), new VFlds(T.OPERATOR, OPT.TB_OBJ, EM.PERSON, P.PE_MOBILE));
		String linesParams[] = toCodes(new VFlds(L.QTY, L.UOM, L.PRICE, L.AMT), new VFlds(L.GOODS, GOODS.CODE), new VFlds(L.GOODS, GOODS.NAME), new VFlds(L.GOODS, GOODS.SPEC));
		new CrtJsp(mainParams, linesParams, SalPresent.class, SalPresentLine.class).run();
	}
	 ...
	 * @param vflds
	 */
	public SalPresentGrf() {
		super(SalPresent.class, null, 8,// int参数表示外围主表行数
				new VFlds(T.ORG, T.CODE, T.CUST, T.CUST_NAME, T.STATUS, T.AMT, T.AMT_COST, T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.SHIPING_MODE, T.OPERATOR, T.REM, COM.TEL1, COM.ADDR, COM.FAX, SHIP.NAME, SHIP.MOBILE));
	...
	public void initItemsSection(GrfItemsSection is, GrfSectionItem ph, GrfSectionItem pf, GrfSectionItem rh, GrfSectionItem rf) {
		...
		rh.AddItem("StaticBox", "orderName", "销  售  赠  送  单", line2.cellLocationSizeAbs(1, 2, 2, 1)).setTextAlignToMiddleCenter().add(FONT_REPORT_HEAD);
		...
		rh.AddItem("MemoBox", "warehouse", "null", line5.cellLocationSizeAbs(1, 4));
		...
		rh.AddName(T.CUST_NAME, line6.cellLocationSizeAbs(1, 1));
		rh.AddValue(T.CUST_NAME, line6.cellLocationSizeAbs(1, 2));
		Sheet line7 = rh.getSheet().newInnerSheet(7, 1, new double[] {0.62}, 3.65, 9.7, 1.6, -1);
		rh.AddItem("StaticBox", "cmb1Name", "送货地/联系人/电话" + root().getLabelExtStr(), line7.cellLocationSizeAbs(1, 1), root().getAlignFleldLabel());
		rh.AddItem("MemoBox", SHIP.ADDR.getFld().getCode(), "[#{" + toCode(new VFlds(T.SHIPING, SHIP.ADDR)) + "}#]/[#{" + toCode(new VFlds(T.SHIPING, SHIP.NAME)) + "}#]/[#{" + toCode(new VFlds(T.SHIPING, SHIP.MOBILE)) + "}#]", line7.cellLocationSizeAbs(1, 2), SHIP.ADDR.getFld().crtVFld().getAlign());
		Sheet line8 = rh.getSheet().newInnerSheet(8, 1, new double[] {0.62}, 2.4, 4.5,1.7,4.8,1.7,-1);
		rh.AddItem("StaticBox", T.OPERATOR.getFld().getCode() + "Name", T.OPERATOR.getFld().getName() + "/手机" + root().getLabelExtStr(), line8.cellLocationSizeAbs(1, 1), root().getAlignFleldLabel());
		rh.AddItem("MemoBox", T.OPERATOR.getFld().getCode(), "[#{" + T.OPERATOR.getFld().getCode() + "}#]/[#{" + toCode(new VFlds(T.OPERATOR, OPT.TB_OBJ, EM.PERSON, P.PE_MOBILE)) + "}#]", line8.cellLocationSizeAbs(1, 2), T.OPERATOR.getFld().crtVFld().getAlign());
		//报表尾相关配置
