...
public class PurPresent extends BeanBill<PurPresent> implements ICstInout{
	public static final Tb TB = new Tb(PurPresent.class, "受赠单", "受赠单").setAutoIncrement()
	    .addActIUDL().addActApprove().addActNote().addActTally().addActPrint();
	...
	public List<IGoods> getCstInoutLines(int type) {
		return CstPub.getCmbGoods(Idu.getLinesTid(this, PurPresentLine.class));
	}
  ...
  //方法----------------------------------------------
  public static PurPresent loadUniqueCode(boolean lockFlag,String code) {
    return (PurPresent)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static PurPresent chkUniqueCode(boolean lockFlag,String code) {
    return (PurPresent)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
