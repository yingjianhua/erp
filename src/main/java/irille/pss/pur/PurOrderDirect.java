...
/**
 * 采购直接单的财务打印，需要显示商品明细信息
 * @author whx
 * @version 创建时间：2014年8月15日 上午9:49:47
 */
public class PurOrderDirect extends BeanBill<PurOrderDirect> implements ICstInvoice, ICstInout {
	public static final Tb TB = new Tb(PurOrderDirect.class, "采购直销单").setAutoIncrement().addActIUDL()
	    .addActOpt("checkPrice", "核价").addActApprove().addActNote().addActTally().addActPrint();
	...
	public List<IGoodsPrice> getInvoiceLines() {
		return Idu.getLinesTid(this, PurOrderDirectLine.class);
	}
  ...
  //方法----------------------------------------------
  public static PurOrderDirect loadUniqueCode(boolean lockFlag,String code) {
    return (PurOrderDirect)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static PurOrderDirect chkUniqueCode(boolean lockFlag,String code) {
    return (PurOrderDirect)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
