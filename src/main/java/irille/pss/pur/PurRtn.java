...
public class PurRtn extends BeanBill<PurRtn> implements ICstInvoice, ICstInout {
	public static final Tb TB = new Tb(PurRtn.class, "采购退货单").setAutoIncrement().addActIUDL()
	    .addActApprove().addActNote().addActTally().addActPrint();
  ...
  public void initTallyLines(TallyLines ls) {
		ls.addByAlias(gtCell(), this, Pur.SubjectAlias.PUR_PENDING.getCode()).set(_amt.negate(), Gl.ODirect.DR, null,
		    "采购退货单(" + getCode() + ")");
  }

	@Override
	public List<IGoodsPrice> getInvoiceLines() {
		return Idu.getLinesTid(this, PurRtnLine.class);
	}

	@Override
	public List<IGoods> getCstInoutLines(int type) {
		return CstPub.getCmbGoods(Idu.getLinesTid(this, PurRtnLine.class));
	}
  ...
  //方法----------------------------------------------
  public static PurRtn loadUniqueCode(boolean lockFlag,String code) {
    return (PurRtn)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static PurRtn chkUniqueCode(boolean lockFlag,String code) {
    return (PurRtn)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
