 ...
 * 冲减定金根据凭证去处理，待定
 * @author whx
 * @version 创建时间：2014年8月27日 上午10:15:31
 */
public class SalSaleDirect extends BeanBill<SalSaleDirect> implements ICstInvoice, ICstInout {
	public static final Tb TB = new Tb(SalSaleDirect.class, "直销单").setAutoIncrement().addActIUDL().addActApprove()
	...
	public List<IGoodsPrice> getInvoiceLines() {
		return Idu.getLinesTid(this, SalSaleDirectLine.class);
	}

	@Override
	public List<IGoods> getCstInoutLines(int type) {
		return CstPub.getCmbGoods(Idu.getLinesTid(this, SalSaleDirectLine.class));
	}
  ...
  //方法----------------------------------------------
  public static SalSaleDirect loadUniqueCode(boolean lockFlag,String code) {
    return (SalSaleDirect)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static SalSaleDirect chkUniqueCode(boolean lockFlag,String code) {
    return (SalSaleDirect)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
