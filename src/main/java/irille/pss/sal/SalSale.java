...
/**
 * 是否根据新的主键变更类型--待研究、EXT如何单据间跳转规则--待研究
 * 销售单、直销单，需要显示产生财务单据的处理状态，在单据主体上显示他方单据状态，列表中不显示
 * 销售单等都增加开票标志
 * @author whx
 * @version 创建时间：2014年8月15日 上午9:22:49
 */
public class SalSale extends BeanBill<SalSale> implements ICstInvoice, ICstInout {
	public static final Tb TB = new Tb(SalSale.class, "销售单").setAutoIncrement().addActIUDL().addActApprove().addActNote()
	    .addActTally().addActOpt("crtGs", "产生出库单").addActPrint();

	public enum T implements IEnumFld {//@formatter:off
		CMB_FORM(CmbBill.fldFlds()),
		CUST(SYS.CUST), //客户
		CUST_NAME(SYS.NAME__100, "客户名称"), //客户名称
		ORD(SalOrder.fldOutKey().setNull()),
		WAREHOUSE(GsWarehouse.fldOutKey()),
		AMT(SYS.AMT), //金额
		AMT_PAY(SYS.AMT, "现付金额"),
		AMT_ORD(SYS.AMT, "冲减订金金额"),
	...
	public List<IGoodsPrice> getInvoiceLines() {
		return Idu.getLinesTid(this, SalSaleLine.class);
	}

	@Override
	public List<IGoods> getCstInoutLines(int type) {
		return CstPub.getCmbGoods(Idu.getLinesTid(this, SalSaleLine.class));
	}
  ...
  private BigDecimal _amtOrd;	// 冲减订金金额  DEC(16,2)
  ...
  //方法----------------------------------------------
  public static SalSale loadUniqueCode(boolean lockFlag,String code) {
    return (SalSale)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static SalSale chkUniqueCode(boolean lockFlag,String code) {
    return (SalSale)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
  ...
  public BigDecimal getAmtOrd(){
    return _amtOrd;
  }
  public void setAmtOrd(BigDecimal amtOrd){
    _amtOrd=amtOrd;
  }
