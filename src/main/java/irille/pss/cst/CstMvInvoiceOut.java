...
public class CstMvInvoiceOut extends BeanBill<CstMvInvoiceOut> {
	public static final Tb TB = new Tb(CstMvInvoiceOut.class, "调出发票汇总单").setAutoIncrement().addActIns().addActDel()
	    .addActList().addActApprove().addActNote().addActTally();

	public enum T implements IEnumFld {//@formatter:off
		CMB_BILL(CmbBill.fldFlds()),
		MV_TYPE(Tb.crt(SalMvOut.OMvType.DEFAULT)),
		AMT(SYS.AMT), //金额
	...
	public void initTallyLines(TallyLines ls) {
		if(gtMvType() == SalMvOut.OMvType.CELL) 
			ls.addByAlias(gtCell(), this, Cst.SubjectAlias.CST_MV_IN.getCode()).set(_amt, Gl.ODirect.CR, null, "调出发票汇总单(" + getCode() + ")");
		else 
			ls.addByAlias(gtCell(), this, Cst.SubjectAlias.CST_MV_OUT.getCode()).set(_amt, Gl.ODirect.CR, null, "调出发票汇总单("+getCode()+")");		
		ls.addByAlias(gtCell(), this, Sal.SubjectAlias.SAL_PENDING.getCode()).set(_amt, Gl.ODirect.DR, null, "调出发票汇总单(" + getCode() + ")");
	}
  ...
  //方法----------------------------------------------
  public static CstMvInvoiceOut loadUniqueCode(boolean lockFlag,String code) {
    return (CstMvInvoiceOut)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstMvInvoiceOut chkUniqueCode(boolean lockFlag,String code) {
    return (CstMvInvoiceOut)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
