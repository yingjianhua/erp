 ...
 * 考虑-总重量，运贷司机
 * @author whx
 * @version 创建时间：2014年8月15日 下午4:57:59
 */
public class CstOut extends BeanBill<CstOut> {
	public static final Tb TB = new Tb(CstOut.class, "出库核算单").setAutoIncrement().addActIns().addActDel().addActList()
	...
	public void initTallyLines(TallyLines ls) {
		...
		for(GlJournal journal : gsMap.keySet()) {
			ls.addByJournal(journal).set(sum.sumAmt(journal),Gl.ODirect.CR,null,"出库核算单("+ getCode()+")").setGoodsLine(gsMap.get(journal));
		}
		for(GlJournal journal : cstMap.keySet()) {
			ls.addByJournal(journal).set(cstMap.get(journal),Gl.ODirect.DR, null, "出库核算单("+ getCode() +")");
		}
  ...
  //方法----------------------------------------------
  public static CstOut loadUniqueCode(boolean lockFlag,String code) {
    return (CstOut)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstOut chkUniqueCode(boolean lockFlag,String code) {
    return (CstOut)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
