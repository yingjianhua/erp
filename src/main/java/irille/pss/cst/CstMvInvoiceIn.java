...
public class CstMvInvoiceIn extends BeanBill<CstMvInvoiceIn> {
	public static final Tb TB = new Tb(CstMvInvoiceIn.class, "调入发票汇总单").setAutoIncrement().addActIns().addActDel()
	    .addActList().addActApprove().addActNote().addActTally();

	public enum T implements IEnumFld {//@formatter:off
		CMB_BILL(CmbBill.fldFlds()),
		MV_TYPE(Tb.crt(PurMvIn.OMvType.DEFAULT)),
		AMT(SYS.AMT), //金额
	...
	public void initTallyLines(TallyLines ls) {
		if (gtMvType() == PurMvIn.OMvType.CELL)
			ls.addByAlias(gtCell(), this, Cst.SubjectAlias.CST_MV_GOODSIN.getCode()).set(_amt, Gl.ODirect.DR, null,
			    "调入发票汇总单(" + getCode() + ")");
		else
			ls.addByAlias(gtCell(), this, Cst.SubjectAlias.CST_MV_GOODSOUT.getCode()).set(_amt, Gl.ODirect.DR, null,
			    "调入发票汇总单(" + getCode() + ")");
		ls.addByAlias(gtCell(), this, Pur.SubjectAlias.PUR_PENDING.getCode()).set(_amt, Gl.ODirect.CR, null,
		    "调入发票汇总单(" + getCode() + ")");
	}
  ...
  //方法----------------------------------------------
  public static CstMvInvoiceIn loadUniqueCode(boolean lockFlag,String code) {
    return (CstMvInvoiceIn)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstMvInvoiceIn chkUniqueCode(boolean lockFlag,String code) {
    return (CstMvInvoiceIn)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
