...
public class CstPurInvoiceRed extends BeanBill<CstPurInvoiceRed> {
	public static final Tb TB = new Tb(CstPurInvoiceRed.class, "采购红字发票汇总单").setAutoIncrement().addActIns().addActDel()
	...
	public void initTallyLines(TallyLines ls) {
		ls.addByAlias(gtCell(), this, Pur.SubjectAlias.PUR_PENDING.getCode()).set(_amt, Gl.ODirect.CR, null,
		    "采购红字发票汇总单(" + getCode() + ")");
		ls.addByAlias(gtCell(), this, Cst.SubjectAlias.CST_PUR.getCode()).set(_amt, Gl.ODirect.DR, null,
		    "采购红字发票汇总单(" + getCode() + ")");
	}
  ...
  //方法----------------------------------------------
  public static CstPurInvoiceRed loadUniqueCode(boolean lockFlag,String code) {
    return (CstPurInvoiceRed)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstPurInvoiceRed chkUniqueCode(boolean lockFlag,String code) {
    return (CstPurInvoiceRed)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
