...
public class CstInRed extends BeanBill<CstInRed> {
	public static final Tb TB = new Tb(CstInRed.class, "红字入库核算单").setAutoIncrement().addActIns().addActDel().addActList().addActApprove()
	...
	public void initTallyLines(TallyLines ls) {
		...
		for(GlJournal journal : gsMap.keySet()) {
			ls.addByJournal(journal).set(sum.sumAmt(journal),Gl.ODirect.DR,null,"红字入库核算单("+ getCode()+")").setGoodsLine(gsMap.get(journal));;
		}
		for(GlJournal journal : cstMap.keySet()) {
			ls.addByJournal(journal).set(cstMap.get(journal),Gl.ODirect.CR, null, "红字入库核算单("+ getCode() +")");
		}
	}
  ...
  //方法----------------------------------------------
  public static CstInRed loadUniqueCode(boolean lockFlag,String code) {
    return (CstInRed)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstInRed chkUniqueCode(boolean lockFlag,String code) {
    return (CstInRed)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
