...
public class CstIn extends BeanBill<CstIn> {
	public static final Tb TB = new Tb(CstIn.class, "入库核算单").setAutoIncrement().addActIns().addActDel().addActList().addActApprove().addActNote()
	    .addActTally();
	...
	public void initTallyLines(TallyLines ls) {
		...
		for(GlJournal journal : gsMap.keySet()) {
			ls.addByJournal(journal).set(sum.sumAmt(journal),Gl.ODirect.DR,null,"入库核算单("+ getCode()+")").setGoodsLine(gsMap.get(journal));
		}
		for(GlJournal journal : cstMap.keySet()) {
			ls.addByJournal(journal).set(cstMap.get(journal),Gl.ODirect.CR, null, "入库核算单("+ getCode() +")");
		}
	}
  ...
  //方法----------------------------------------------
  public static CstIn loadUniqueCode(boolean lockFlag,String code) {
    return (CstIn)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static CstIn chkUniqueCode(boolean lockFlag,String code) {
    return (CstIn)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
