...
public class EGsRequest extends GsRequest {
	public static void main(String[] args) {
		new EGsRequest().crtExt().crtFiles();
	}

	public EMCrt crtExt() {
		GsRequestLine.TB.getCode(); // 解决静态块初始化异常的问题。。。
		VFlds[] vflds = new VFlds[] { new VFlds(TB) };
		VFlds[] mflds = new VFlds[] { new VFlds(T.CODE, T.WAREHOUSE, T.STATUS, T.ORG, T.DEPT) }; // 主表信息字段
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds,
				new VFlds[] { new VFlds(GsRequestLine.T.PKEY) });
		...
		vs.moveAfter(T.REM, T.WAREHOUSE);//form中的备注字段移动到仓库后面
//		vs.get(T.WAREHOUSE).setReadOnly("!this.insFlag");
		VFlds vsl = ext.getVfldsList();
		vsl.moveBefore(T.WAREHOUSE, T.STATUS);

		ext.newExts();
		((EMZipWin)ext.newZipWin(null)).setWidth(630);
		ext.init();
		return ext;
	}
