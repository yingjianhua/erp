...
public class EGsUnite extends GsUnite
{

  public static void main(String[] args)
  {
    new EGsUnite().crtExt().crtFiles();
  }

  public EMCrt crtExt()
  {
    GsUniteLine.TB.getCode(); // 解决静态块初始化异常的问题。。。
  ...
  class MyForm extends EMFormTwoRow<MyForm>
  {
    ...
    {
      if (fld.getCode().equals(GsUnite.T.GOODS.getFld().getCode()))
      {
        fldList.add(new myGoods());
      } else if (fld.getCode().equals(GsUnite.T.UOM.getFld().getCode()))
  ...
  public static class myGoods implements IExtOut
  {
    ...
    {
      buf.append(new EMModel(GsUnite.TB).loadFunCode(EGsUnite.class, "myGoods"));
    }
  ...
  public static class myUom implements IExtOut
  {
    ...
    public void out(int tabs, StringBuilder buf)
    {
      buf.append(new EMModel(GsUnite.TB).loadFunCode(EGsUnite.class, "myUom"));
    }
