...
public class EGsOut extends GsOut {
	public static void main(String[] args) {
		new EGsOut().crtExt().crtFiles();
	}

	public MyComp crtExt() {
		GsOutLineView.TB.getCode();
		CmbGoods.TB.getCode();
		...
		VFlds[] mflds = new VFlds[] { new VFlds(T.CODE, T.ORIG_FORM_NUM, T.GS_NAME, T.WAREHOUSE, T.STATUS, T.OUT_TIME,
		    T.CREATED_BY, T.CREATED_TIME, T.ORG) }; // 主表信息字段
		VFlds[] searchVflds = new VFlds[] { new VFlds(T.ORIG_FORM_NUM, T.WAREHOUSE, T.STATUS) }; // 搜索栏字段
		// 在上面改对MODEL、LIST、FROM是同时影响，下面则是各改各的-互不影响
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(GsOutLineView.T.PKEY) });
		...
		vl.setExpandAndHidden("true", SysShiping.T.ADDR, SysShiping.T.MOBILE, SysShiping.T.TEL, SysShiping.T.TIME_ARR_PLAN,
		    SysShiping.T.TIME_SHIP_PLAN, T.REM, T.APPR_BY, T.APPR_TIME, T.CREATED_BY, T.CREATED_TIME, T.DEPT, T.OPERATOR,
		    T.CHECKER);
	...
	class MyZipList extends EMZipList<EMZipList> {
		...
		public void initFunsAddAct(ExtAct act) {
			if (!act.getCode().equals("doAppr"))
				...
				add(AddFun("onDoAppr", EGsOut.class));
				add(AddFun("onApproveRecord", EGsIn.class).addFunParasExp("form, data"));
				add(AddFun("getGoodsKeys", EGsOut.class).addFunParasExp("pkey"));
				add(AddFun("onApprWin", EGsOut.class).addFunParasExp("selection"));
			}
		}

		@Override
		public void initFuns() {
			AddFun("onChangeStatus", EGsIn.class).addFunParasExp("status");
			super.initFuns();
		}
	...
	class MyComp extends EMCrtComp<MyComp> {
		...
		public void crtOutFld(VFld fld) {
			...
			if (fld.getTb().getClazz().equals(GsOutLineView.class))
				el.getVFlds().del(GsOutLineView.T.BATCH_CODE);
			addExt(el);
