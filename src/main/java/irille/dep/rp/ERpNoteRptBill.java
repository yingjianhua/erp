...
import irille.gl.rp.RpNotePayBill.T;
...
public class ERpNoteRptBill extends RpNoteRptBill{
	
	public static void main(String[] args) {
	  new ERpNoteRptBill().crtExt().crtFiles();
  }
	...
	public EMCrt crtExt() {
		...
		vf.moveAfter(T.TYPE, T.TRAN_TIME);
		vf.setHidden("true", T.RECEIVE_DATE, T.NAME);
		vf.moveLast(T.REM);
	...
	class MyList extends EMList<MyList> {
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (oact == OAct.DO_APPR || oact == OAct.UN_APPR || oact == OAct.DO_TALLY || oact == OAct.UN_TALLY)
				v = new ExtAct(this, act, ERpNoteRptBill.class, getPack(), getClazz(), getTb().getName());
			else if (oact == OAct.DO_NOTE)
				v = new ExtAct(this, act, ERpNoteRptBill.class, getTb().getClazz().getName());
			else {
		...
		public void initFuns() {
			AddFun("onChangeStatus", ERpNoteRptBill.class).addFunParasExp("status");
			ExtFunDefine fun = AddList("listeners") // 表格行选择事件，主要设置按钮是否可用
			    .AddFunDefine("selectionchange", new ExtExp("selModel, selected"));
			fun.add(loadFunCode(ERpNoteRptBill.class, "initChange"));
