...
public class ECstOut extends CstOut {
	...
	public void crtExt() {
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstOutLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstOut.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstOut.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstOut.class);
