...
public class ECstMvInvoiceIn extends CstMvInvoiceIn {
	public static void main(String[] args) {
		new ECstMvInvoiceIn().crtExt();
	}

	public void crtExt() {
		CmbGoods.TB.getCode();
		CstMvInvoiceInLine.TB.getCode();
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstMvInvoiceInLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstMvInvoiceIn.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstMvInvoiceIn.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstMvInvoiceIn.class);
