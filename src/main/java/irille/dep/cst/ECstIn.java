...
public class ECstIn extends CstIn {
	...
	public void crtExt() {
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstInLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstIn.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstIn.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstIn.class);
