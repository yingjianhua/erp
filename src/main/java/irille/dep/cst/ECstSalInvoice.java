...
public class ECstSalInvoice extends CstSalInvoice {
	public static void main(String[] args) {
		new ECstSalInvoice().crtExt();
	}

	public void crtExt() {
		CmbGoods.TB.getCode();
		CstSalInvoiceLine.TB.getCode();
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstSalInvoiceLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstSalInvoice.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstSalInvoice.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstSalInvoice.class);
