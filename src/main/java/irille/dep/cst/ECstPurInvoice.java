...
public class ECstPurInvoice extends CstPurInvoice {
	public static void main(String[] args) {
		new ECstPurInvoice().crtExt();
	}

	public void crtExt() {
		CmbGoods.TB.getCode();
		CstPurInvoiceLine.TB.getCode();
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstPurInvoiceLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstPurInvoice.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstPurInvoice.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstPurInvoice.class);
