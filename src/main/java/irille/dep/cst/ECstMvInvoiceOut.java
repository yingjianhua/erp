...
public class ECstMvInvoiceOut extends CstMvInvoiceOut {
	public static void main(String[] args) {
		new ECstMvInvoiceOut().crtExt();
	}

	public void crtExt() {
		CmbGoods.TB.getCode();
		CstMvInvoiceOutLine.TB.getCode();
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstMvInvoiceOutLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstMvInvoiceOut.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstMvInvoiceOut.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstMvInvoiceOut.class);
