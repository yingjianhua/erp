...
public class ECstPurInvoiceRed extends CstPurInvoiceRed {
	public static void main(String[] args) {
		new ECstPurInvoiceRed().crtExt();
	}

	public void crtExt() {
		CmbGoods.TB.getCode();
		CstPurInvoiceRedLine.TB.getCode();
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(CstPurInvoiceRedLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ECstPurInvoiceRed.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ECstPurInvoiceRed.class).addFunParasExp("status");
			super.initFuns();
		}
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (act.getCode().equals("ins")) {
				ExtAct v = new ExtAct(this, act, ECstPurInvoiceRed.class);
