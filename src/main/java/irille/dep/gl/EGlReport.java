...
public class EGlReport extends GlReport{
	public static void main(String[] args) {
		//new EGlReport().crtExt().crtFiles();
	}

	public EMCrt crtExt() {
		//CmbGoods.TB.getCode();
		GlReportLine.TB.getCode();
		VFlds[] vflds =  new VFlds[]{new VFlds(TB)};
		VFlds[] searchVflds = new VFlds[] {new VFlds(T.CODE)}; // 搜索栏字段
		// 在上面改对MODEL、LIST、FROM是同时影响，下面则是各改各的-互不影响
		MyComp ext = new MyComp(TB, vflds, vflds, searchVflds, new VFlds[]{new VFlds(GlReportLine.TB)});
		ext.newExts().init();
		return ext;
	}
