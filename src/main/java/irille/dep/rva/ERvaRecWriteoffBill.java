...
import irille.dep.pya.EPyaPayWriteoffBill;
...
public class ERvaRecWriteoffBill extends RvaRecWriteoffBill {
	public static void main(String[] args) {
		new ERvaRecWriteoffBill().crtExt().crtFiles();
		;
	}
	...
	class MyForm extends EMForm<MyForm> {
		...
		public void initComponent(ExtFunDefine fun) {
			fun.add(loadFunCode(ERvaRecWriteoffBill.class, "initComponentMy",getPack(),getClazz()));
			//@formatter:off
	...
	class MyList extends EMList<MyList> {
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (oact == OAct.DO_APPR || oact == OAct.UN_APPR || oact == OAct.DO_TALLY || oact == OAct.UN_TALLY)
				v = new ExtAct(this, act, EPyaPayWriteoffBill.class, getPack(), getClazz(), getTb().getName());
			else if (oact == OAct.DO_NOTE)
				v = new ExtAct(this, act, EPyaPayWriteoffBill.class, getTb().getClazz().getName());
			else {
		...
		public void initFuns() {
			AddFun("onChangeStatus", EPyaPayWriteoffBill.class).addFunParasExp("status");
			ExtFunDefine fun = AddList("listeners") // 表格行选择事件，主要设置按钮是否可用
			    .AddFunDefine("selectionchange", new ExtExp("selModel, selected"));
			fun.add(loadFunCode(EPyaPayWriteoffBill.class, "initChange"));
