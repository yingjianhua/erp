...
import irille.dep.pya.EPyaPayBill;
...
public class ERvaRecBill extends RvaRecBill {
	public static void main(String[] args) {
		new ERvaRecBill().crtExt().crtFiles();
		;
	}
	...
	class MyForm extends EMForm<MyForm> {
		...
		public void initComponent(ExtFunDefine fun) {
			fun.add(loadFunCode(ERvaRecBill.class, "initComponentMy",getPack(),getClazz()));
			//@formatter:off
	...
	class MyList extends EMList<MyList> {
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			...
			if (oact == OAct.DO_APPR || oact == OAct.UN_APPR || oact == OAct.DO_TALLY || oact == OAct.UN_TALLY)
				v = new ExtAct(this, act, EPyaPayBill.class, getPack(), getClazz(), getTb().getName());
			else if (oact == OAct.DO_NOTE)
				v = new ExtAct(this, act, EPyaPayBill.class, getTb().getClazz().getName());
			else {
