 ...
 */
public class ESysSupplier extends SysSupplier {
	IEnumFld pya = SYS.AMT;
	public static void main(String[] args) {
		new ESysSupplier().crtExt().crtFiles();
	}

	public EMCrt crtExt() {
		SysCustomOrg.TB.getCode();
		pya.getFld().setName("应付款").setCode("pyaAmt");
		...
		// 下面是编辑相关JS产生
		VFlds editVflds = new VFlds().addWithout(SysSupplier.TB, SysSupplier.T.PKEY, SysSupplier.T.REM);
		VFlds[] editOuts = new VFlds[] {
		    new VFlds().add(SysPersonLink.TB, SysPersonLink.T.TYPE, SysPersonLink.T.NAME, SysPersonLink.T.PE_MOBILE,
		        SysPersonLink.T.PE_EMAIL, SysPersonLink.T.OF_FAX, SysPersonLink.T.OF_ADDR),new VFlds(SysSupplierOrg.TB) };
		VFld[] outflds = new VFld[] { SysPersonLink.T.TB_OBJ_LONG.getFld().getVFld(),
		    SysSupplierOrg.T.SUPPLIER.getFld().getVFld() };
	...
	public class MyComp extends EMCrtSimpleTwo {
		...
		public ExtFile newModel() {
			return new EMModel(getTb(), getVfldsModel(), new VFlds(pya));
		}
	}
