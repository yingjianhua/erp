...
public class ESalSaleDirect extends SalSaleDirect {
	public static void main(String[] args) {
		new ESalSaleDirect().crtExt();
	}
	...
	public void crtExt() {
		...
		MyComp ext = new MyComp(TB, vflds, mflds, searchVflds, new VFlds[] { new VFlds(SalSaleDirectLine.T.PKEY) });
	...
	class MyZipList extends EMZipList<MyZipList> {
		...
		public void initFormListMain() {
			...
			l.addExp("listeners", loadFunCode(ESalSaleDirect.class, "initChange", getOutVFlds().get(0).getCode()));
		}

		public void initFuns() {
			AddFun("onChangeStatus", ESalSaleDirect.class).addFunParasExp("status");
			super.initFuns();
			add(AddFun("onInsOrderLoad", ESalSaleDirect.class));
			add(AddFun("onInsOrder", ESalSaleDirect.class).addFunParas(new ExtExp("data, params")));
		}
		//@formatter:off	
...
*** End initChange *********/
	...
	class MyForm extends EMFormTwoRow<MyForm> {
		...
		public void setFldAttr(VFld fld, ExtList fldList) {
			if (fld.getCode().equals(SalSaleDirect.T.CUST.getFld().getCode())) {
				fldList.add(new ESalOrder.MyCust());
			} else if (fld.getCode().equals(SalSaleDirect.T.SHIPING_MODE.getFld().getCode())) {
	...
	class MyComp extends EMCrtComp<MyComp> {
		...
		public VFlds getNowOutKeyZipListFormVflds() {
			...
			vs.del(SalSaleDirectLine.T.EXPENSES_SALE);
			return vs;
		}
