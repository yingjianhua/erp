...
public class ESalMvOut extends SalMvOut {
	public static void main(String[] args) {
		new ESalMvOut().crtExt();
	}

	public void crtExt() {
		SalMvOutLine.TB.getCode(); // 解决静态块初始化异常的问题。。。
		CmbGoods.TB.getCode();
		...
		VFlds[] mflds = new VFlds[] { new VFlds(T.CODE, T.STATUS, T.AMT,T.WAREHOUSE, T.ORG_OTHER,
		    T.CELL_OTHER, T.FROM_TYPE, T.GOODS_FROM, T.ORG) }; // 主表信息字段
		...
		fvs.del(T.CODE, T.STATUS, T.ORG, T.DEPT, T.CREATED_BY, T.CREATED_TIME, T.APPR_BY, T.APPR_TIME,
		    T.AMT_COST, T.GOODS_FROM, T.IN_FORM, T.ORG_OTHER, T.MV_TYPE, T.CELL, T.SHIPING, T.TALLY_BY, T.TALLY_TIME);
		fvs.setHidden("true", T.WAREHOUSE_OTHER, T.FROM_TYPE);
		fvs.get(T.CELL_OTHER).setReadOnly("this.insFd || this.chkFlag");
		...
		ext.getVfldsForm().setHidden("true", SysShiping.T.TIME_ARR_PLAN,
		    ...
		    SysShiping.T.TEL);
		ext.getVfldsForm().setReadOnly("true", SysShiping.T.NAME, SysShiping.T.ADDR,
	...
	class MyComp extends EMCrtComp<MyComp> {
		...
		public ExtFile newZipListForm(Tb tb, VFld outfld, VFlds... vflds) {
			...
			// return null;
			// VFlds lfv = vflds[0];
			// lfv.del(SalMvOutLine.T.PRICE, SalMvOutLine.T.AMT,
			// SalMvOutLine.T.AMT_COST);
	...
	class MyListForm extends EMZipListForm<MyListForm> {
		...
		public void initComponent(ExtFunDefine fun) {
			// 主界面功能定义
			fun.add(loadFunCode(ESalMvOut.class, "initComponentGoods"));
			//@formatter:off	
	...
	class MyListMain extends EMZipListMain<MyListMain> {
		...
		public void initFuns() {
			...
			if (super.isUpd()) {
				...
				ExtFunDefine onUpdWin = AddFun("onUpdWin", ESalMvOut.class, getPack(), getClazz())
			...
			ExtFunDefine onChkWin = AddFun("onChkWin", ESalMvOut.class, getPack(), getClazz())
			    .addFunParasExp("selection");
			add(onChkWin);
		}
	...
	class MyList extends EMZipList<MyList> {
		...
		@Override
		public void initFuns() {
			AddFun("onChangeStatus", EPurPresent.class).addFunParasExp("status,ordStatus,fromType");
			AddFun("onInsFd", ESalMvOut.class);
			super.initFuns();
			//@formatter:off	
		...
		public void loadTbAct(Class funCodeFile, Act act) {
			if (act.getCode().equals("unAppr")) {
				ExtAct v = new ExtAct(this, act, ESalMvOut.class, getPack(), getClazz(), getTb().getName());
			...
			if (act.getCode().equals("ins")) {
				...
				.addExp("menu", "[{text:'从需求新增',iconCls : 'ins-icon', scope : this,handler: this.onInsFd}]");
				getActs().add(v);
				return;
			}
			super.loadTbAct(funCodeFile, act);
//			if (act.getCode().equals("insFd")) {
//				ExtAct v = new ExtAct(this, act, ESalMvOut.class);
//				v.add(TEXT, act.getName()).add(ICON_CLS, "ins-icon")
//				    .addExp("itemId", "this.oldId+'" + act.getCode() + "'").add(SCOPE, EXP_THIS)
//				    .addExp(HANDLER, "this.on" + act.getCodeFirstUpper());
//				getActs().add(v);
//			}
			if (act.getCode().equals("cOut")) {
				ExtAct v = new ExtAct(this, act, ESalMvOut.class);
				v.add(TEXT, act.getName()).add(ICON_CLS, "ins-icon")
				    .addExp("itemId", "this.oldId+'" + act.getCode() + "'").add(SCOPE, EXP_THIS)
				    .addExp(HANDLER, "this.on" + act.getCodeFirstUpper()).addExp("disabled", "this.lock");
		...
		*** End onInsFd *********/
		...
		if (selection){
			Ext.Ajax.request({
				url : base_path+'/sal_SalMvOut_cOut?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
...
*** End onUnAppr *********/
	...
	//@formatter:on	
		...
		public void initFormListMain() {
			...
			l.addExp("listeners",
			    loadFunCode(ESalMvOut.class, "initFormListMainStatus", getOutVFlds().get(0).getCode()));
		}
		//@formatter:off	
	...
	class MyWin extends EMZipWin<MyWin> {
		...
		public void initFuns() {
			// TODO Auto-generated method stub
			AddFun("setActiveRecordIns", ESalMvOut.class).addFunParasExp("warehouse", "lines");
			super.initFuns();
		}
	...
	*** End setActiveRecordIns *********/
	...
	class MyForm extends EMFormTwoRow<MyForm> {
		...
		public void setFldAttr(VFld fld, ExtList fldList) {
			if (fld.getCode().equals(SalMvOut.T.WAREHOUSE.getFld().getCode())) {
				fldList.add(new MyWarehouse()).setCloseStr(null);
			} else if (fld.getCode().equals(SalMvOut.T.SHIPING_MODE.getFld().getCode())) {
		...
		@Override
		public void initComponent(ExtFunDefine fun) {
			fun.add(loadFunCode(ESalMvOut.class, "initComponentForm"));
			//@formatter:off
			/** Begin initComponentForm ********
			  if(this.chkFlag)
		...
		class MyWarehouse implements IExtOut {
			...
			public void out(int tabs, StringBuilder buf) {
				buf.append(new EMModel(SalMvOut.TB).loadFunCode(ESalMvOut.class, "MyWarehouse"));
			}
...
//@formatter:off	
	...
	class MyShip implements IExtOut {
		...
		public void out(int tabs, StringBuilder buf) {
			buf.append(new EMModel(SalMvOut.TB).loadFunCode(ESalMvOut.class, "myShip"));
		}
