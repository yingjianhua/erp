...
import irille.gl.pya.Pya.OPdType;
...
public class PyaNoteDepositPayable extends BeanLong<PyaNoteDepositPayable> implements IWrite{
	private static final Log LOG = new Log(PyaNoteDepositPayable.class);

	public static final Tb TB = new Tb(PyaNoteDepositPayable.class, "预付账款凭条")
			.addActIUDL();

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF(CmbWriteoff.fldCmbFlds()), 
		TYPE(TB.crt(OPdType.DEFAULT)),
...
//			TB.addIndex("journalStateTallyDate",false,JOURNAL,STATE,TALLY_DATE);
	...
	public void tallyCancel(GlNote note) {
		if (getBalance().compareTo(note.getAmt()) != 0 && Idu.chkBeUsed(PyaNoteDepositPayableLine.class, PyaNoteDepositPayableLine.T.MAIN_NOTE, getPkey()))
  ...
  private Byte _type;	// 预付账款类型 <OPdType>  BYTE
	// SUPPLIER:1,供应商
  ...
  private Short _rowVersion;	// 版本  SHORT
	...
	@Override
  public PyaNoteDepositPayable init(){
		super.init();
    ...
    _type=OPdType.DEFAULT.getLine().getKey();	// 预付账款类型 <OPdType>  BYTE
    _obj=null;	// 对象  LONG
  ...
  public OPdType gtType(){
    return (OPdType)(OPdType.SUPPLIER.getLine().get(_type));
  }
  public void stType(OPdType type){
