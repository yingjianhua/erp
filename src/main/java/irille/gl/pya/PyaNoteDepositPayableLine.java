...
public class PyaNoteDepositPayableLine extends BeanLong<PyaNoteDepositPayableLine> implements IWriteoff {
	public static final Log LOG = new Log(PyaNoteDepositPayableLine.class);
	public static final Tb TB = new Tb(PyaNoteDepositPayableLine.class, "预付账款核销凭条");

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF_LINE(CmbWriteoffLine.fldFlds()), 
		MAIN_NOTE(Tb.crtOutKey(PyaNoteDepositPayable.class, "mainNote", "预付账款")),
	...
	public void tallyWriteoff(GlNote note) {
		PyaNoteDepositPayable write = loadAndLock(PyaNoteDepositPayable.class, getMainNote());
  ...
  private Long _mainNote;	// 预付账款 <表主键:PyaNoteDepositPayable>  LONG
  private Short _rowVersion;	// 版本  SHORT

	@Override
  public PyaNoteDepositPayableLine init(){
		super.init();
    _tallyDate=null;	// 记账日期  DATE
    _mainNote=null;	// 预付账款 <表主键:PyaNoteDepositPayable>  LONG
  ...
  public PyaNoteDepositPayable gtMainNote(){
    if(getMainNote()==null)
      return null;
    return (PyaNoteDepositPayable)get(PyaNoteDepositPayable.class,getMainNote());
  }
  public void stMainNote(PyaNoteDepositPayable mainNote){
    if(mainNote==null)
      setMainNote(null);
    else
