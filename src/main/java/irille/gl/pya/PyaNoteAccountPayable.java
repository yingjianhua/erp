...
public class PyaNoteAccountPayable extends BeanLong<PyaNoteAccountPayable> implements IWrite {
	private static final Log LOG = new Log(PyaNoteAccountPayable.class);

	public static final Tb TB = new Tb(PyaNoteAccountPayable.class, "应付账款凭条").addActIUDL();

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF(CmbWriteoff.fldCmbFlds()), 
		TYPE(TB.crt(OPaType.DEFAULT)),
...
//			TB.addIndex("journalStateTallyDate",false,JOURNAL,STATE,TALLY_DATE);
	...
	public void tallyCancel(GlNote note) {
		if (getBalance().compareTo(note.getAmt()) != 0 && Idu.chkBeUsed(PyaNoteAccountPayableLine.class, PyaNoteAccountPayableLine.T.MAIN_NOTE, getPkey()))
  ...
  private Byte _type;	// 应付账款类型 <OPaType>  BYTE
	// SUPPLIER:1,供应商
  ...
  private Short _rowVersion;	// 版本  SHORT
	...
	@Override
  public PyaNoteAccountPayable init(){
		super.init();
    ...
    _type=OPaType.DEFAULT.getLine().getKey();	// 应付账款类型 <OPaType>  BYTE
    _obj=null;	// 对象  LONG
  ...
  public OPaType gtType(){
    return (OPaType)(OPaType.SUPPLIER.getLine().get(_type));
  }
  public void stType(OPaType type){
