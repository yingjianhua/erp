...
import irille.gl.gl.Gl.OTransformType;
...
public class GlTransform extends BeanBill<GlTransform> {
	private static final Log LOG = new Log(GlTransform.class);

	public static final Tb TB = new Tb(GlTransform.class, "内转单", BeanBill.TB).setAutoIncrement().addActIUDL()
	    .addActApprove().addActNote().addActTally();

	public enum T implements IEnumFld {//@formatter:off
		CMB_BILL(CmbBill.fldFlds()),
		TYPE(TB.crt(Gl.OTransformType.NORMAL)),
  ...
  private Byte _type;	// 类型 <OTransformType>  BYTE
	// NORMAL:1,普通
	// CARRY:2,损益结转
	// SUM:3,汇总入账
  ...
  public GlTransform init(){
    ...
    _type=OTransformType.DEFAULT.getLine().getKey();	// 类型 <OTransformType>  BYTE
    _rowVersion=0;	// 版本  SHORT
    return this;
  }

  //方法----------------------------------------------
  public static GlTransform loadUniqueCode(boolean lockFlag,String code) {
    return (GlTransform)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static GlTransform chkUniqueCode(boolean lockFlag,String code) {
    return (GlTransform)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
  ...
  public OTransformType gtType(){
    return (OTransformType)(OTransformType.NORMAL.getLine().get(_type));
  }
  public void stType(OTransformType type){
