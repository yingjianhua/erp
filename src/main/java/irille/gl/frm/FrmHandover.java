...
/**
 * 建表，单据是否需要交接；资源权限
 * 交接单明细中的单据选择时，只能从待处理单据中把属于自己的交出去
 * 审核-只能由接收人操作，把对应待处理单据中的当前用户更改为接收人
 * @author whx
 * @version 创建时间：2014年8月27日 上午8:49:05
 */
public class FrmHandover extends BeanForm<FrmHandover> implements IExtName {
	public static final Tb TB = new Tb(FrmHandover.class, "单据交接单").setAutoIncrement().addActIUDL().addActApprove().addActOpt("take", "接收");
	...
	public enum T implements IEnumFld {//@formatter:off
		...
		ORG(TB.get("org")),	//机构
		DEPT(TB.get("dept")),	//部门
		CREATED_BY(TB.get("createdBy")),	//交出人
		CREATED_TIME(TB.get("createdTime")),	//交出时间
		APPR_BY(TB.get("apprBy")),	//接收人
		APPR_TIME(TB.get("apprTime")),	//接收时间
		REM(TB.get("rem")),	//备注
	...
	static { //在此可以加一些对FLD进行特殊设定的代码
		T.CREATED_BY.getFld().setName("交出人");
		T.APPR_BY.getFld().setName("接收人");
		T.CREATED_TIME.getFld().setName("交出时间");
		T.APPR_TIME.getFld().setName("接收时间");
  ...
  private Integer _createdBy;	// 交出人 <表主键:SysUser>  INT
  private Date _createdTime;	// 交出时间  TIME
  private Integer _apprBy;	// 接收人 <表主键:SysUser>  INT<null>
  private Date _apprTime;	// 接收时间  TIME<null>
  private String _rem;	// 备注  STR(200)<null>
  private Short _rowVersion;	// 版本  SHORT

	@Override
  public FrmHandover init(){
		super.init();
    ...
    _createdBy=Idu.getUser().getPkey();	// 交出人 <表主键:SysUser>  INT
    _createdTime=Env.getTranBeginTime();	// 交出时间  TIME
    _apprBy=null;	// 接收人 <表主键:SysUser>  INT
    _apprTime=null;	// 接收时间  TIME
  ...
  //方法----------------------------------------------
  public static FrmHandover loadUniqueCode(boolean lockFlag,String code) {
    return (FrmHandover)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static FrmHandover chkUniqueCode(boolean lockFlag,String code) {
    return (FrmHandover)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
