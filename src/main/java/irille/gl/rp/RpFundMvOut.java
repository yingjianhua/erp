...
public class RpFundMvOut extends BeanBill<RpFundMvOut> {
	public static final Tb TB = new Tb(RpFundMvOut.class, "资金调出单").setAutoIncrement().addActIUDL().addActApprove()
	    .addActOpt("doOut", "调出").addActOpt("unOut", "调出取消").addActNote().addActTally();
	...
	public enum T implements IEnumFld {//@formatter:off
		...
		MVIN(RpFundMvIn.fldOutKey().setNull()),
		CASHIER(SYS.CASHIER),
		IN_TIME(SYS.TIME__NULL, "调出时间"),
		IS_APPR(SYS.NY, "调入审核标识"),
		IS_CRT(SYS.NY, "是否发起方"),
  ...
  private Long _mvin;	// 资金调入单 <表主键:RpFundMvIn>  LONG<null>
  private Integer _cashier;	// 出纳 <表主键:SysUser>  INT
  private Date _inTime;	// 调出时间  TIME<null>
  private Byte _isAppr;	// 调入审核标识 <OYn>  BYTE
	// YES:1,是
	// NO:0,否
  ...
  public RpFundMvOut init(){
    ...
    _mvin=null;	// 资金调入单 <表主键:RpFundMvIn>  LONG
    _cashier=null;	// 出纳 <表主键:SysUser>  INT
    _inTime=null;	// 调出时间  TIME
    _isAppr=OYn.DEFAULT.getLine().getKey();	// 调入审核标识 <OYn>  BYTE
  ...
  //方法----------------------------------------------
  public static RpFundMvOut loadUniqueCode(boolean lockFlag,String code) {
    return (RpFundMvOut)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static RpFundMvOut chkUniqueCode(boolean lockFlag,String code) {
    return (RpFundMvOut)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
  ...
  public void setMvin(Long mvin){
    _mvin=mvin;
  }
  public RpFundMvIn gtMvin(){
    if(getMvin()==null)
      return null;
    return (RpFundMvIn)get(RpFundMvIn.class,getMvin());
  }
  public void stMvin(RpFundMvIn mvin){
    if(mvin==null)
      setMvin(null);
    else
      setMvin(mvin.getPkey());
  }
