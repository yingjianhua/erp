...
import irille.gl.rva.Rva.ORdType;
...
public class RvaNoteDeposit extends BeanLong<RvaNoteDeposit> implements IWrite{
	private static final Log LOG = new Log(RvaNoteDeposit.class);

	public static final Tb TB = new Tb(RvaNoteDeposit.class, "预收账款凭条")
			.addActIUDL();

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF(CmbWriteoff.fldCmbFlds()), 
		TYPE(TB.crt(ORdType.DEFAULT)),
...
//			TB.addIndex("journalStateTallyDate",false,JOURNAL,STATE,TALLY_DATE);
	...
	public void tallyCancel(GlNote note) {
		if (getBalance().compareTo(note.getAmt()) != 0 && Idu.chkBeUsed(RvaNoteDepositLine.class, RvaNoteDepositLine.T.MAIN_NOTE, getPkey()))
  ...
  private Byte _type;	// 预收账款类型 <ORdType>  BYTE
	// CUST:1,客户
  ...
  public RvaNoteDeposit init(){
    ...
    _type=ORdType.DEFAULT.getLine().getKey();	// 预收账款类型 <ORdType>  BYTE
    _obj=null;	// 对象  LONG
  ...
  public ORdType gtType(){
    return (ORdType)(ORdType.CUST.getLine().get(_type));
  }
  public void stType(ORdType type){
