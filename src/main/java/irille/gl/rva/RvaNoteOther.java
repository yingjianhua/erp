...
import irille.gl.rva.Rva.ORoType;
...
public class RvaNoteOther extends BeanLong<RvaNoteOther> implements IWrite{
	private static final Log LOG = new Log(RvaNoteOther.class);

	public static final Tb TB = new Tb(RvaNoteOther.class, "其它应收款凭条").addActIUDL();

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF(CmbWriteoff.fldCmbFlds()), 
		TYPE(TB.crt(ORoType.DEFAULT)),
...
//			TB.addIndex("journalStateTallyDate",false,JOURNAL,STATE,TALLY_DATE);
	...
	public void tallyCancel(GlNote note) {
		if (getBalance().compareTo(note.getAmt()) != 0 && Idu.chkBeUsed(RvaNoteOtherLine.class, RvaNoteOtherLine.T.MAIN_NOTE, getPkey()))
  ...
  private Byte _type;	// 其它应收款类型 <ORoType>  BYTE
	// USER:1,职员
  ...
  public RvaNoteOther init(){
    ...
    _type=ORoType.DEFAULT.getLine().getKey();	// 其它应收款类型 <ORoType>  BYTE
    _obj=null;	// 对象  LONG
  ...
  public ORoType gtType(){
    return (ORoType)(ORoType.USER.getLine().get(_type));
  }
  public void stType(ORoType type){
