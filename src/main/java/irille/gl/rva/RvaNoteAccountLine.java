...
public class RvaNoteAccountLine extends BeanLong<RvaNoteAccountLine> implements IWriteoff {
	public static final Log LOG = new Log(RvaNoteAccountLine.class);
	public static final Tb TB = new Tb(RvaNoteAccountLine.class, "应收账款核销凭条");

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF_LINE(CmbWriteoffLine.fldFlds()), 
		MAIN_NOTE(Tb.crtOutKey(RvaNoteAccount.class, "mainNote", "应收账款")),
	...
	public void tallyWriteoff(GlNote note) {
		RvaNoteAccount write = loadAndLock(RvaNoteAccount.class, getMainNote());
  ...
  private Long _mainNote;	// 应收账款 <表主键:RvaNoteAccount>  LONG
  private Short _rowVersion;	// 版本  SHORT

	@Override
  public RvaNoteAccountLine init(){
		super.init();
    _tallyDate=null;	// 记账日期  DATE
    _mainNote=null;	// 应收账款 <表主键:RvaNoteAccount>  LONG
  ...
  public RvaNoteAccount gtMainNote(){
    if(getMainNote()==null)
      return null;
    return (RvaNoteAccount)get(RvaNoteAccount.class,getMainNote());
  }
  public void stMainNote(RvaNoteAccount mainNote){
    if(mainNote==null)
      setMainNote(null);
    else
