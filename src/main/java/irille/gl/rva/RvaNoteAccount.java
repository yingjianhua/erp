...
public class RvaNoteAccount extends BeanLong<RvaNoteAccount> implements IWrite{
	private static final Log LOG = new Log(RvaNoteAccount.class);

	public static final Tb TB = new Tb(RvaNoteAccount.class, "应收账款凭条")
			.addActIUDL();

	public enum T implements IEnumFld {//@formatter:off
		CMB_WRITEOFF(CmbWriteoff.fldCmbFlds()), 
		TYPE(TB.crt(ORaType.DEFAULT)),
...
//			TB.addIndex("journalStateTallyDate",false,JOURNAL,STATE,TALLY_DATE);
	...
	public void tallyCancel(GlNote note) {
		if (getBalance().compareTo(note.getAmt()) != 0 && Idu.chkBeUsed(RvaNoteAccountLine.class, RvaNoteAccountLine.T.MAIN_NOTE, getPkey()))
  ...
  private Byte _type;	// 应收账款类型 <ORaType>  BYTE
	// CUST:1,客户
  ...
  public RvaNoteAccount init(){
    ...
    _type=ORaType.DEFAULT.getLine().getKey();	// 应收账款类型 <ORaType>  BYTE
    _obj=null;	// 对象  LONG
  ...
  public ORaType gtType(){
    return (ORaType)(ORaType.CUST.getLine().get(_type));
  }
  public void stType(ORaType type){
