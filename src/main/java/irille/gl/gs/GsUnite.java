...
public class GsUnite extends BeanForm<GsUnite> implements ICstInout{
	private static final Log LOG = new Log(GsUnite.class);

	public static final Tb TB = new Tb(GsUnite.class, "合并单", "合并单").setAutoIncrement().addActIUDL().addActApprove();

	public enum T implements IEnumFld {//@formatter:off
		CMB_FORM(CmbForm.fldFlds()),
		WAREHOUSE(GsWarehouse.fldOutKey()),
		GOODS(GsGoods.fldOutKey().setName("货物")),
		QTY(SYS.QTY),
		UOM(GsUom.fldOutKey()),
	...
	public List<IGoods> getCstInoutLines(int type) {
		if(type==ICstInout.TYPE_IN) {
			Vector<IGoods> list = new Vector<IGoods>();
			GsUniteLine goods = new GsUniteLine();
			goods.setGoods(getGoods());
			goods.setQty(getQty());
			goods.stUom(gtUom());
			list.add(goods);
			return list;
		} else if(type==ICstInout.TYPE_OUT) {
			 return Idu.getLinesTid(this, GsUniteLine.class);			
		}
		return null;
  ...
  //方法----------------------------------------------
  public static GsUnite loadUniqueCode(boolean lockFlag,String code) {
    return (GsUnite)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static GsUnite chkUniqueCode(boolean lockFlag,String code) {
    return (GsUnite)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
