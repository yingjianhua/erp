...
public class GsMovement extends BeanForm<GsMovement> {
	public static final Tb TB = new Tb(GsMovement.class, "内部调拨单").setAutoIncrement().addActIUDL().addActApprove();

	public enum T implements IEnumFld {//@formatter:off
		CMB_FORM(CmbForm.fldFlds()),
		WAREHOUSE_OUT(GsWarehouse.fldOutKey().setName("调出仓库")),
		WAREHOUSE_IN(GsWarehouse.fldOutKey().setName("调入仓库")),
  ...
  private Integer _warehouseOut;	// 调出仓库 <表主键:GsWarehouse>  INT
  private Integer _warehouseIn;	// 调入仓库 <表主键:GsWarehouse>  INT
  private Short _rowVersion;	// 版本  SHORT

	@Override
  public GsMovement init(){
		super.init();
    ...
    _warehouseOut=null;	// 调出仓库 <表主键:GsWarehouse>  INT
    _warehouseIn=null;	// 调入仓库 <表主键:GsWarehouse>  INT
    _rowVersion=0;	// 版本  SHORT
    return this;
  }

  //方法----------------------------------------------
  public static GsMovement loadUniqueCode(boolean lockFlag,String code) {
    return (GsMovement)loadUnique(T.IDX_CODE,lockFlag,code);
  }
  public static GsMovement chkUniqueCode(boolean lockFlag,String code) {
    return (GsMovement)chkUnique(T.IDX_CODE,lockFlag,code);
  }
  public Long getPkey(){
    return _pkey;
  }
  ...
  public GsWarehouse gtWarehouseOut(){
    if(getWarehouseOut()==null)
      return null;
    return (GsWarehouse)get(GsWarehouse.class,getWarehouseOut());
  }
  public void stWarehouseOut(GsWarehouse warehouseOut){
    if(warehouseOut==null)
      setWarehouseOut(null);
    else
      setWarehouseOut(warehouseOut.getPkey());
  }
  ...
  public GsWarehouse gtWarehouseIn(){
    if(getWarehouseIn()==null)
      return null;
    return (GsWarehouse)get(GsWarehouse.class,getWarehouseIn());
  }
  public void stWarehouseIn(GsWarehouse warehouseIn){
    if(warehouseIn==null)
      setWarehouseIn(null);
    else
      setWarehouseIn(warehouseIn.getPkey());
  }
