...
import irille.dep.gs.EGsOut;
import irille.pub.doc.DocTran;
import irille.pub.html.EMCrt;

public class GsOutDoc extends DocTran {
	public static EMCrt EXT=new EGsOut().crtExt();
	public static DocTran DOC=new GsOutDoc().init();
...
//		public static  DocNoun NOUN_cell=DOC.newNoun("NOUN_cell", "核算单元"); //名词定义
	...
	@Override
	public void initMsg() {
		DOC.p2("出库单由销售出库、调拨出库、盘盈出库、拆分合并出库等业务形成。系统中的出库单都是由其它单据自动产生的，库管员不用手工录入，只有审核与弃审操作。");
...
//				"设置核算单元。核算单元相对于其它财务系统就是一本独立的账套。").add(getUrl(DExample.ACT_del)).add(getUrl(DExample.DOC));
	...
	public static DocFld FLD_m_outTime=DOC.getFld("m_outTime");	//实际出库时间
	public static DocTb TB_m=DOC.getTb("TB_m");	//出库单
